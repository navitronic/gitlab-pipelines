...
func (c *Client) FetchPipelinesBySHA(ctx context.Context, projectID int, userID int, sha string) ([]gitlab.Pipeline, error) {
	endpoint := fmt.Sprintf("projects/%d/pipelines?sha=%s&per_page=20", projectID, sha)
	if userID > 0 {
		endpoint += "&username=" + strconv.Itoa(userID)
	}
	return c.fetchPipelines(ctx, endpoint)
}
...
func (c *Client) FetchPipelinesByRef(ctx context.Context, projectID int, userID int, ref string) ([]gitlab.Pipeline, error) {
	endpoint := fmt.Sprintf("projects/%d/pipelines?ref=%s&per_page=20", projectID, ref)
	if userID > 0 {
		endpoint += "&username=" + strconv.Itoa(userID)
	}
	return c.fetchPipelines(ctx, endpoint)
...
func (c *Client) fetchPipelines(ctx context.Context, endpoint string) ([]gitlab.Pipeline, error) {
	out, err := c.Run(ctx, "api", endpoint)
	if err != nil {
